...
import md5 from "md5";
import { randomicToken } from "../util/Util.js";

const token = randomicToken();
...
async function main(nome, email, hash) {
  ...
  let mensa = "<h5>Troca de Senha</h5>";
  mensa += `<h6>Estimado: ${nome}</h6>`;
  mensa += "<h6>Você solicitou a troca de senha: ";
  mensa += `O seu token para troca de senha ${token}</h6>`;
  ...
  let info = await transporter.sendMail({
    from: '"Troca de Senha" <resetpassword@email.com>', // sender address
